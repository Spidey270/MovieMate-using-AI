            ...
            try {
                ...
                if (currentUser) {
                    ...
                    if (isFriend) {
                        ...
                        const isPending = requests.data.find(r => r.sender_id === id || r.receiver_id === id)
            ...
            <div className="pt-24 mx-auto max-w-4xl px-4">
                <div className="bg-zinc-900 rounded-2xl p-8 shadow-xl border border-white/5">
                                ...
                                {currentUser && currentUser.id !== id && (
                                    <>
                                        {friendStatus === 'accepted' ? (
                                            ...
                                            <Button onClick={handleAddFriend} className="bg-primary hover:bg-red-700">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8 border-t border-gray-800">
                        <div>
                            <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-widest mb-4">Favorite Genres</h3>
                            <div className="flex flex-wrap gap-2">
                                {user.favorite_genres?.length > 0 ? (
                                    user.favorite_genres.map(g => (
                                        <span key={g} className="px-3 py-1 bg-zinc-800 rounded-full text-xs text-gray-300">
                                            {g} {/* Note: These are IDs, ideally we should resolve them to names */}
                                        </span>
                                    ))
                                ) : (
                        ...
                        <div>
                            <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-widest mb-4">Top Languages</h3>
                            <div className="flex flex-wrap gap-2">
                                {user.favorite_languages?.length > 0 ? (
                                    user.favorite_languages.map(l => (
                                        <span key={l} className="px-3 py-1 bg-zinc-800 rounded-full text-xs text-gray-300">{l}</span>
                                    ))
                                ) : (
                                    <span className="text-gray-600 text-sm italic">No languages selected</span>
                                )}
                            </div>
                        </div>
                    </div>
