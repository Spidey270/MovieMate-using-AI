    ...
    return (
        <div className="min-h-screen bg-secondary pb-20">
            ...
            <div className="relative z-20 -mt-32 space-y-8 pl-4 md:pl-12">
                ...
                {user && (
                    <section>
                        <div className="flex items-center justify-between mb-4 pr-8">
                            <h2 className="text-xl font-semibold text-white md:text-2xl">Recommended For You</h2>
                            <button 
                                onClick={async () => {
                                    try {
                                        await api.post("/recommendations/generate");
                                        alert("AI is analyzing your profile! You will receive a notification when it's done.");
                                    } catch (err) {
                                        console.error(err);
                                    }
                                }}
                        ...
                        {recommendations.length > 0 ? (
                            ...
                            <div className="text-gray-500 text-sm">No recommendations yet. Click generate to get started!</div>
                        )}
                    </section>
                )}
                ...
                {/* Trending Row */}
                <section>
                    <h2 className="mb-4 text-xl font-semibold text-white md:text-2xl">Trending Now</h2>
                    <div className="flex gap-4 overflow-x-scroll pb-4 scrollbar-hide">
                        {trending.map(movie => (
                            <MovieCard key={movie.id} movie={movie} />
                        ))}
                ...
                {genres.slice(0, 3).map(genre => (
                    <section key={genre.id}>
                        <h2 className="mb-4 text-xl font-semibold text-white md:text-2xl">{genre.name} Movies</h2>
                        <div className="flex gap-4 overflow-x-scroll pb-4 scrollbar-hide">
                            {/* Filter trending by genre for demo, or fetch specific genre endpoint later */}
                            {trending.filter(m => m.genre_ids.includes(genre.id)).map(movie => (
                                <MovieCard key={movie.id} movie={movie} />
                            ))}
                            {/* Fallback if empty */}
                            {trending.filter(m => m.genre_ids.includes(genre.id)).length === 0 && (
                                <p className="text-sm text-gray-500">No movies in this genre yet.</p>
                            )}
                        </div>
                    </section>
                ))}
