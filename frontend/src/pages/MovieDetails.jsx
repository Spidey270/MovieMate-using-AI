    ...
    return (
        <div className="min-h-screen bg-secondary text-white pb-20">
            ...
            <div className="relative h-[50vh] w-full">
                <img src={backdropUrl} className="h-full w-full object-cover opacity-60" alt="Backdrop" />
            ...
            <div className="mx-auto -mt-32 max-w-6xl px-4 relative z-10">
                <div className="flex flex-col gap-8 md:flex-row">
                    ...
                    <div className="flex-grow pt-8">
                        ...
                        <div className="flex items-center gap-4 text-gray-400 text-sm mb-6">
                            ...
                            <span className="text-green-500 font-bold">{movie.imdb_rating} IMDb</span>
                        </div>

                        <div className="flex flex-wrap gap-2 mb-6">
                            {movie.genres?.map(g => (
                                <span key={g.id} className="rounded-full bg-gray-800 px-3 py-1 text-xs">{g.name}</span>
                            ))}
                        </div>
                ...
                <div className="mt-16 max-w-4xl">
                    ...
                    {/* Add Review */}
                    {user ? (
                        <form onSubmit={handleSubmitReview} className="mb-10 bg-gray-900 p-6 rounded-lg">
                        ...
                        <div className="mb-10 bg-gray-900 p-6 rounded-lg text-center">
                            <p className="text-gray-300">Please <a href="/login" className="text-primary underline">login</a> to leave a review.</p>
                        </div>
                    )}
                    ...
                    <div className="space-y-6">
                        {reviews.map((review, index) => (
                            <div key={review.id || index} className="border-b border-gray-800 pb-6">
                                <div className="flex items-center justify-between mb-2">
                                    <div className="flex items-center gap-3">
                                        <Link to={`/user/${review.user_id}`} className="flex h-8 w-8 items-center justify-center rounded-full bg-zinc-700 font-bold hover:bg-primary transition">
                                            {review.username[0].toUpperCase()}
                                        </Link>
                                        <div>
                                            <Link to={`/user/${review.user_id}`} className="font-bold hover:text-primary transition">
                                                {review.username}
                                            </Link>
